...
<nav
  class="relative max-w-screen-lg mx-auto flex items-center justify-between px-4 sm:px-6"
>
  <div class="flex items-center flex-1">
    <div class="flex items-center justify-between w-full md:w-auto">
      <a style="text-decoration: none;" aria-label="Home" href="/"
        ><img class="h-8 w-auto sm:h-10" src="logo.png" alt="Logo" /></a
      ><a
        style="text-decoration: none;"
        class="text-xl font-bold leading-6 text-white sm:truncate ml-2"
        href="/">Noava</a
      >
  ...
  <div class="hidden space-x-10 items-center md:flex content-center">
    <a
      style="text-decoration: none;"
      href="https://analytics.sushi.com"
      target="_blank"
      rel="noopener noreferrer"
      class="font-medium text-white hover:text-gray-300 transition duration-150 ease-in-out"
      on:mouseenter={() => (show_elm = true)}
      on:mouseleave={() => (show_elm = false)}
      >Socials
      <Tooltip pointer="middle" position="bottom" show={show_elm}>
        <div class="flex space-x-6 md:order-2 ml-2 mt-4 ">
          <a
            href="https://sushichef.medium.com/"
            class="text-gray-400 hover:text-gray-300"
            ><span class="sr-only">Medium</span><svg
              class="h-6 w-6"
    ...
    <a
      ...
      class="inline-flex items-center text-sm leading-5 font-medium text-white"
      ><div class="sc-bdVaJa dBEbmu">Enter App</div></a
    >
  </div>
</nav>
